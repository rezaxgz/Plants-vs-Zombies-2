...
public final class ProjectSelfTest {
    ...
    private void checkDifficultyMultipliers() {
        DifficultyRules normal =
                DifficultyRules.forLevel(3);
        DifficultyRules hard =
                DifficultyRules.forLevel(5);
        requireClose(1.0,
    ...
    private static void checkWaveProgression(
        ...
        require(second.getDifficulty() * 4
                        == first.getDifficulty() * 5,
                "wave two must be 25 percent harder");
        require(last.getDifficulty()
                        == second.getDifficulty() * 2,
    ...
    private void checkScoredGameCheats() {
        ...
        game.releaseNuke();
        require(game.getBoard().getZombies().size()
                        == zombieCount,
    ...
    private void checkTimedWarSystems() {
        ...
        zombie.kill();
        TimedWarSystem killSystem =
                TimedWarSystem.forZombieKills(30.0, 1);
        require(killSystem.update(
                        0.0, List.of(zombie), List.of())
                        == TimedWarState.SUCCEEDED,
                "kill objective did not succeed");

        TimedWarSystem sunSystem =
                TimedWarSystem.forSunProduction(60.0, 50);
