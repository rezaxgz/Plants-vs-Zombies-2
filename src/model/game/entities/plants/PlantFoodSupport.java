...
public final class PlantFoodSupport {
    ...
    public static boolean supports(BasePlant plant) {
        ...
        if (plant instanceof SunProducer) {
            return ((SunProducer) plant).getType()
                    != SunProducerPlantType.GOLD_BLOOM;
        }
        ...
        if (plant instanceof Shooter) {
            return ((Shooter) plant).getType()
                    != ShooterPlantType.APPEASE_MINT;
        }
        if (plant instanceof Lobber) {
            return ((Lobber) plant).getType()
                    != LobberPlantType.ARMA_MINT;
        }
        ...
        if (plant instanceof Homing) {
            return ((Homing) plant).getType()
                    != HomingPlantType.CAT_TAIL_MINT;
        }
        if (plant instanceof Melee) {
            return ((Melee) plant).getType()
                    != MeleePlantType.ENFORCE_MINT;
        }
        return plant instanceof Wallnut
                && ((Wallnut) plant).getType()
                        != WallnutPlantType.REINFORCE_MINT;
    }
}
