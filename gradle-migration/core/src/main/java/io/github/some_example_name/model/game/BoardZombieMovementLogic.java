...
import io.github.some_example_name.model.game.entities.other.ArcadeMachine;
...
import io.github.some_example_name.model.game.entities.other.RollingBarrel;
...
import io.github.some_example_name.model.game.entities.zombies.abilities.ArcadePushAbility;
import io.github.some_example_name.model.game.entities.zombies.abilities.BarrelPushAbility;
import io.github.some_example_name.model.game.entities.zombies.abilities.IceBlockPushAbility;
import io.github.some_example_name.model.game.entities.zombies.abilities.LaunchAbility;
import io.github.some_example_name.model.game.entities.zombies.abilities.PianoCrushAbility;
import io.github.some_example_name.model.game.entities.zombies.abilities.ZombieAbility;
...
abstract class BoardZombieMovementLogic extends BoardModifierLogic {
    ...
    void applySliderTile(Zombie zombie) {
        ...
        if (tile == null
                || (tile.getTileType() != TileType.SLIDER_UP
                        && tile.getTileType() != TileType.SLIDER_DOWN)) {
            zombie.clearSliderTrigger();
            return;
        }
        ...
        int laneDelta = tile.getTileType() == TileType.SLIDER_UP ? -1 : 1;
    ...
    void crushPlantsWithPushedObstacle(
        ...
        for (BasePlant plant : new ArrayList<>(getPlants())) {
            if (plant.getEntityPosition() == null
                    || plant.getEntityPosition().getRow() != obstacle.getLane()
                    || Math.abs(
                            plant.getEntityPosition().getColumn()
                                    - obstacle.getColumnPosition()) > PushedObstacle.COLLISION_RADIUS_TILES) {
                continue;
            }
    ...
    void updateReverseMovingProspector(
            Zombie prospector, float deltaSeconds) {
        BasePlant target = findNearestPlantToRight(prospector);
        if (target == null) {
            prospector.moveRight(deltaSeconds,
                    numberOfColumns + PROJECTILE_BOARD_MARGIN);
            if (prospector.getColumnPosition() >= numberOfColumns - POSITION_EPSILON) {
                prospector.kill();
        ...
        double attackColumn = target.getEntityPosition().getColumn()
                - Zombie.ATTACK_REACH;
        if (prospector.getColumnPosition() + POSITION_EPSILON >= attackColumn) {
    ...
    BasePlant findNearestPlantToRight(Zombie zombie) {
        ...
        for (BasePlant plant : getPlants()) {
            ...
            if (column + POSITION_EPSILON >= zombie.getColumnPosition()
        ...
        for (BasePlant plant : new ArrayList<>(getPlants())) {
            if (plant.getEntityPosition() == null
                    ...
                    || Math.abs(plant.getEntityPosition().getColumn()
                            - block.getColumnPosition()) > IceBlock.COLLISION_RADIUS_TILES) {
                continue;
            }
            plant.takeDamage(Integer.MAX_VALUE);
    ...
    void updateHypnotizedZombie(
        ...
        PushedObstacle obstacle = findNearestPushedObstacleToRight(zombie);

        if (obstacle != null
                && (target == null
                        || obstacle.getColumnPosition() <= target.getColumnPosition())) {
    ...
    PushedObstacle findNearestPushedObstacleToRight(
        ...
        for (PushedObstacle obstacle : getPushedObstacles()) {
            ...
            if (obstacle.getLane() != zombie.getLane()
                    || column + Zombie.ATTACK_REACH < zombie.getColumnPosition()
                    || column >= nearestColumn) {
                continue;
            }
            nearest = obstacle;
    ...
    Zombie findNearestHostileZombieToRight(Zombie hypnotizedZombie) {
        ...
        for (Zombie zombie : getZombies()) {
            ...
            if (column + Zombie.ATTACK_REACH >= hypnotizedZombie.getColumnPosition() - POSITION_EPSILON
                    && column < nearestColumn) {
                nearest = zombie;
                nearestColumn = column;
            }
