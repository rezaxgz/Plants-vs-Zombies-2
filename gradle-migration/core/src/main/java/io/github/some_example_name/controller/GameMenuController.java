...
import io.github.some_example_name.model.game.PlantFoodResult;
import io.github.some_example_name.model.game.PlantPlacementResult;
import io.github.some_example_name.model.game.RewardCollectionResult;
import io.github.some_example_name.model.game.SunCollectionResult;
...
public final class GameMenuController {
    ...
    public static CommandResult handleAdvanceTime(Matcher matcher) {
        ...
        List<String> progressResults = synchronizeAdventureProgress();
    ...
    public static CommandResult handleShowAvailablePlants(
        ...
        List<String> available = game.getLockedPlantTypes();
    ...
    public static CommandResult handleShowForcedPlants(
        ...
        List<String> forced = game.getForcedPlantTypes();
        StringBuilder output = new StringBuilder("forced plants");
        if (forced.isEmpty()) {
            output.append(System.lineSeparator())
                    .append("- none");
        } else {
    ...
    public static CommandResult handleShowProtectedPlants(
        ...
        StringBuilder output = new StringBuilder("protected plants");
        for (ProtectedPlantStatus status : game.getProtectedPlantStatuses()) {
            output.append(System.lineSeparator())
    ...
    public static CommandResult handlePlantFromConveyor(
        ...
        ConveyorPlantPacket packet = game.getConveyorPacket(index);
        ConveyorPlacementResult result = game.plantFromConveyor(index, position);
    ...
    private static List<String> synchronizeAdventureProgress() {
        Menu currentMenu = App.getInstance().getCurrentMenu();
        if (!(currentMenu instanceof GameMenu)) {
            return List.of();
        }
    ...
    private static void persistConsumedGreenhouseBoosts(
            Game game) {
        List<String> consumed = game.drainConsumedGreenhouseBoosts();
        if (consumed.isEmpty()) {
            return;
        }
        User user = App.getInstance().getLoggedInUser();
